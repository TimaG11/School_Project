...
# Проверяет, лежит ли точка на вертикальном или горизонтальном отрезке.
...
# Считает общую длину ломаной по клеткам сетки.
...
# Разрезает ломаную в заданной точке на две части.
...
# ----------------------- Formatters -----------------------
# Красиво переводит сопротивление в строку с нужными единицами.
...
# Красиво переводит напряжение в строку с нужными единицами.
...
# Красиво переводит ток в строку с нужными единицами.
...
# Красиво переводит мощность в строку с нужными единицами.
...
# Красиво переводит емкость в строку с нужными единицами.
...
# Красиво переводит заряд в строку с нужными единицами.
...
# Красиво переводит энергию в строку с нужными единицами.
...
# Показывает число в нужном формате или знак вопроса, если значения нет.
...
# Готовит строки тока, напряжения и мощности для вывода в интерфейс.
...
# ----------------------- Param model -----------------------
# Хранит один параметр элемента: либо число, либо переменную.
@dataclass
class ParamValue:
    ...
    # Проверяет, задан ли параметр как переменная.
    def is_var(self) -> bool:
        return self.mode == "var"

    # Возвращает число, если параметр не является переменной.
    def numeric(self) -> Optional[float]:
        return None if self.is_var() else float(self.value)


# Показывает параметр как число с единицами или как имя переменной.
...
# Рисует иконку лампочки для кнопки смены темы.
...
# Обычный список, у которого отключено изменение колесиком мыши.
class NoWheelComboBox(QtWidgets.QComboBox):
    # Игнорирует колесико мыши, чтобы значение не менялось случайно.
    def wheelEvent(self, e: QtGui.QWheelEvent) -> None:
        e.ignore()


# Поле числа, у которого отключено изменение колесиком мыши.
...
# Виджет для ввода параметра: число с единицами или переменная.
class ParamInputWidget(QtWidgets.QWidget):
    # Создает объект и задает его начальные настройки.
    def __init__(
        self,
    ...
    # Переключает интерфейс между режимом числа и переменной.
    def _sync_mode(self) -> None:
        md = self.mode.currentData()
        self.stack.setCurrentIndex(0 if md == "value" else 1)

    # Заполняет поля текущим значением параметра.
    ...
    # Считывает параметр из интерфейса и возвращает его в виде объекта.
...
# Окно редактирования параметров элемента.
class ParamsDialog(QtWidgets.QDialog):
    ...
    # Возвращает значения, которые пользователь ввел в окне.
    def values(self) -> Dict[str, ParamValue]:
        return {k: w.get_param() for k, w in self._widgets.items()}


# Окно настройки амперметра или вольтметра.
...
# Окно настройки состояния ключа.
...
# Простое окно для показа длинного текста.
...
# Хранит данные инструмента, которые перетаскиваются на поле.
@dataclass
class ToolPayload:
    tool: str
    ...
    # Упаковывает данные для drag and drop.
    ...
    # Читает данные инструмента из drag and drop.
    @staticmethod
...
# Картинка инструмента, которую можно перетащить на схему.
...
# Левая панель со всеми элементами схемы и их настройками.
class ToolboxWidget(QtWidgets.QWidget):
    toggleRequested = QtCore.Signal()

    # Создает левую панель и все элементы, которые можно перетаскивать на схему.
...
в установившемся режиме ток через него не идет")
    ...
    # Меняет цвета под светлую или темную тему.
...
# Шапка панели отчета, за которую ее можно двигать мышью.
...
# Плавающая панель с общей информацией по схеме.
class ReportPanel(QtWidgets.QFrame):
    ...
    # Запоминает, что панель сдвинул пользователь.
    def mark_user_moved(self) -> None:
        self._user_moved = True

    # Показывает, двигал ли пользователь панель вручную.
    def user_moved(self) -> bool:
        return self._user_moved

    # Двигает панель, но не дает ей выйти за границы окна.
    ...
    # Подправляет положение панели, если она вышла за границы окна.
    ...
    # Показывает новый текст отчета на панели.
...
# Панель с данными по выделенным элементам.
class SelectionDetailsPanel(QtWidgets.QFrame):
    ...
    # Показывает, раскрыта ли панель сейчас.
    def is_expanded(self) -> bool:
        return self._expanded

    # Возвращает высоту панели в свернутом виде.
    def collapsed_height(self) -> int:
        return self.header_btn.sizeHint().height() + 16

    # Возвращает высоту панели в развернутом виде.
    def expanded_height(self) -> int:
        return 220

    # Меняет текст внутри панели.
...
# ----------------------- Scene -----------------------
# Сцена, где живут элементы схемы, сетка и связи между ними.
class CircuitScene(QtWidgets.QGraphicsScene):
    # Создает сцену, настраивает сетку и подготавливает хранилища элементов и расчетов.
    ...
    # Меняет тему сцены и обновляет элементы.
    def set_theme(self, dark: bool) -> None:
        self.dark_theme = bool(dark)
        self.invalidate_analysis()
        self.update()

    # Сбрасывает старые результаты расчета.
    ...
    # Регистрирует новый элемент внутри сцены.
    ...
    # Находит группу связанных элементов.
    ...
    # Объединяет две группы связанных элементов.
    ...
    # Возвращает все элементы из одной группы.
    ...
    # Сдвигает всю группу элементов по сетке.
    ...
    # Пересобирает структуру связей между элементами.
    ...
    # Обновляет все точки соединения после изменений.
    ...
    # Собирает элементы, которые надо посчитать по текущему выделению.
    ...
    # Переводит координаты сетки в координаты сцены.
    ...
    # Закрывает всплывающее окно соединения.
    ...
    # Проверяет, не пора ли предложить соединить элементы.
    ...
    # Создает маленькое окно с кнопкой соединения.
    ...
    # Подтверждает соединение двух элементов.
    ...
    # Добавляет точку соединения между элементами.
    ...
    # Добавляет провод с готовой ломаной линией.
    ...
    # Ищет провода, которые проходят через точку.
    ...
    # Ищет элементы, у которых контакт стоит в этой точке.
    ...
    # Проверяет, можно ли поставить узел в выбранную точку.
    ...
    # Ставит узел и при необходимости разрезает провод.
...
# ----------------------- View -----------------------
# Окно просмотра сцены: зум, перемещение, drag and drop и верхние панели.
class CircuitView(QtWidgets.QGraphicsView):
    createSchemeRequested = QtCore.Signal()
    themeToggled = QtCore.Signal(bool)

    # Создает вид сцены, включает зум, панораму и вспомогательные панели.
    ...
    # Переключает тему по нажатию кнопки.
    def _toggle_theme(self) -> None:
        self.themeToggled.emit(not self._dark)

    # Расставляет панели и кнопки поверх поля.
    ...
    # Обновляет расположение панелей после изменения размера окна.
    def resizeEvent(self, e: QtGui.QResizeEvent) -> None:
        super().resizeEvent(e)
        self._layout_overlays()

    # Меняет масштаб просмотра.
    ...
    # Показывает панель с отчетом.
    def show_report(self, text: str) -> None:
        self.report_panel.set_report(text)
        self.refresh_selection_panel()
        self._layout_overlays()

    # Собирает заголовок для описания выделенного элемента.
    ...
    # Готовит подробный текст по одному выделенному элементу.
    ...
    # Обновляет панель с данными по выделению.
    ...
    # Разрешает перетаскивание инструмента на поле.
    ...
    # Обновляет перетаскивание, пока курсор движется над полем.
    ...
    # Создает новый элемент после отпускания на поле.
...
class CircuitItem(QtWidgets.QGraphicsObject):
    ...
    # Создает базовый элемент схемы и включает выделение, фокус и работу мышью.
    ...
    # Возвращает имя цвета для панели выделения.
    ...
    # Возвращает текущий цвет элемента с учетом темы.
    ...
    # Добавляет в меню список доступных цветов.
    ...
    # Добавляет кнопку поворота так, чтобы меню сразу не закрывалось.
...
# Провод: хранит ломаную линию, умеет тянуться, ветвиться и менять параметры.
class WireItem(CircuitItem):
    # Создает провод и сохраняет его точки на сетке.
    ...
    # Проверяет, проходит ли провод через точку сетки.
    ...
    # Возвращает границы элемента для отрисовки.
    ...
    # Возвращает область, по которой удобно кликать мышью.
    ...
    # Проверяет, попал ли клик в конец провода.
    ...
    # Открывает окно редактирования параметров элемента.
    ...
    # Показывает контекстное меню элемента.
    ...
    # Поворачивает элемент из контекстного меню.
    def _rotate_from_menu(self) -> None:
        self.rotate_90()
        self.scene_ref.refresh_all_junction_geometry()

    # Создает ответвление от провода.
    ...
    # Меняет размеры провода с привязкой к сетке.
    ...
    # Двигает конец провода по сетке.
    ...
    # Упрощает форму провода, убирая лишние точки.
    @staticmethod
...
# База для двухконтактных элементов, которые можно поворачивать.
class TwoTerminalRotatableItem(CircuitItem):
    # Создает двухконтактный элемент с позицией и углом поворота.
    def __init__(self, scene: "CircuitScene"):
        super().__init__(scene)
        self.orientation = 0
        self.center: Point = (0, 0)

    # Возвращает направление элемента по его повороту.
    def _dir(self) -> Point:
...
class ResistorItem(TwoTerminalRotatableItem):
    # Создает резистор и сохраняет его параметры.
...
# Источник напряжения на схеме.
class SourceItem(TwoTerminalRotatableItem):
    # Создает источник и сохраняет его параметры.
...
# Узел соединения нескольких элементов.
class NodeItem(CircuitItem):
    # Создает узел в одной точке сетки.
...
# Общая логика измерительных приборов.
class MeterItem(TwoTerminalRotatableItem):
    ...
    # Создает измерительный прибор и сохраняет его сопротивление.
    ...
    # Возвращает численное сопротивление прибора с учетом режима идеальности.
    def effective_resistance_numeric(self) -> Optional[float]:
        if self.ideal:
            return float(self.IDEAL_R)
        return self.R.numeric()

    # Возвращает сопротивление прибора в виде текста.
...
class CapacitorItem(TwoTerminalRotatableItem):
    ...
    # Создает конденсатор и сохраняет его емкость.
...
# Ключ, который может пропускать ток или разрывать цепь.
class SwitchItem(TwoTerminalRotatableItem):
    TITLE = "Ключ"

    # Создает ключ и запоминает, открыт он или закрыт.
    def __init__(self, scene: "CircuitScene", start: Point, is_open: bool):
        super().__init__(scene)
        self.center = (start[0] + 1, start[1])
        self.is_open = bool(is_open)

    # Возвращает текстовое состояние ключа.
...
# ----------------------- Analysis -----------------------
# Вспомогательная структура для объединения связанных точек.
class PointDSU:
    # Создает пустую структуру для объединения связанных точек.
    def __init__(self):
        self.parent: Dict[Point, Point] = {}

    # Добавляет новую точку в структуру объединения.
...
# Главная функция расчета схемы: собирает связи, решает уравнения и готовит ответ.
...
class MainWindow(QtWidgets.QMainWindow):
    # Собирает главное окно: поле схемы, панель инструментов, меню и тему.
    def __init__(self):
        super().__init__()
    ...
    # Показывает или скрывает левую панель инструментов.
    def _toggle_toolbox(self) -> None:
        self.dock.setVisible(not self.dock.isVisible())

    # Запускает расчет схемы и показывает результат.
